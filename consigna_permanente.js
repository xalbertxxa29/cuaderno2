document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try{
      ...
      let fotoURL=null;
      if(pendingPhoto) fotoURL = await uploadTo(`consignas/permanente/${CLIENTE}/${UNIDAD}/${userId}_${stamp}_foto.jpg`, pendingPhoto);

      let firmaURL=null,firmaData=null;
      if(!sigPad.isEmpty()){
        const dataURL=sigPad.toDataURL('image/png');
        try{ firmaURL=await uploadTo(`consignas/permanente/${CLIENTE}/${UNIDAD}/${userId}_${stamp}_firma.png`, dataURLtoBlob(dataURL)); }
